...
public class Table implements Iterable<Record>, Closeable {
  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
    ...
    public boolean hasNext() {
      //Every data page begins with an n-byte bitmap followed by m records.
      //The bitmap indicates which records in the page are valid.
      //Thus, in order to determine whether or not the next record is valid,
      //we check from our current position onward.
      //If there is a location with a 1 then there is something next!
      //If not, we must return false.
      if (bitmap != null) {
        for (int i = current; i < 8 * bitmap.length; i++) {
          if (Bits.getBit(bitmap, i) == Bits.Bit.ONE) {
            return true;
          }
        }
    ...
    public RecordId next() {
      //Ensure that we can even find one!
      //if(!hasNext()) {
      //  return null;
      //}
      //Every data page begins with an n-byte bitmap followed by m records.
      //The bitmap indicates which records in the page are valid.
      //Search through all the bits until we find a bit that is value 1.

      //Update our current counter. Continue to do this until we reach our first 1.
    ...
    public void mark() {
      ...
      //We iterate one more time in our next method, so we must take our current and subtact one from it.
      //If mark becomes negative doing so, then we have never gone through our next method even once.
      //We must set mark to 0 then.
    ...
    public void reset() {
      ...
      //For our implementation, doing so is as easy as setting current to mark
      //However, if mark() has never been called, we must do nothing.
      //We determine this by checking if mark is negative.
      //This is because mark will only ever be negative during initialization.
      //Everytime we call mark() mark gets set to be a nonegative number.
      if (!(mark < 0)) {
        current = mark;
      }
      //throw new UnsupportedOperationException("hw3: TODO");
  ...
  public class RIDBlockIterator implements BacktrackingIterator<RecordId> {
    //The block we are looking at. Allows you to go through the various pages.
    private BacktrackingIterator<Page> block = null;
    //Iterates through the various RecordIds in the given Page.
    ...
    public boolean hasNext() {
      //If we are currently looking at a page, and it has another record
      if (blockIter != null && blockIter.hasNext())
        return true;
      //If we are not currently looking at a page, or it doesn't have another record
      //Get next page to start searching through.
      if (block != null && block.hasNext()) {
        blockIter = new RIDPageIterator(block.next());
        return hasNext();
      }
      //Exhausted all possibilities. Return false :(
      return false;
      //throw new UnsupportedOperationException("hw3: TODO");
    }
    ...
    public RecordId next() {
      if (hasNext()) {
        //If we called hasNext and found a next page, blockIter will already be primed to return it
        //Return the record Id
        return blockIter.next();
      }
      //Nothing we can give, so return null
      return null;
      //throw new UnsupportedOperationException("hw3: TODO");
    }
     ...
     */
    public void mark() {
      //Mark everything. Store the blockIter state
      block.mark();
      blockIter.mark();
      markedBlockIter = blockIter;
    }
     ...
     */
    public void reset() {
      //Restore everything in opposite order.
      blockIter = markedBlockIter;
      block.reset();
      //Advance the block by one.
      block.next();
      blockIter.reset();
    }
  }
